...
const AdminPlanningWeek: React.FC = () => {
  ...
  const [blockedSlotsData, setBlockedSlotsData] = useState<Map<string, { id: number; reason: string }>>(new Map());
  const [showBlockModal, setShowBlockModal] = useState(false);
  const [showUnblockModal, setShowUnblockModal] = useState(false);
  const [selectedBlockedSlot, setSelectedBlockedSlot] = useState<{ key: string; id: number; date: Date; time: string; reason: string } | null>(null);
  ...
  const getAppointmentsForSlot = (date: Date, timeSlot: TimeSlot): Appointment[] => {
    ...
    return appointments.filter(apt => {
      ...
      try {
        ...
        return aptDateStr === dateStr && aptHour === timeSlot.hour && aptMinute === timeSlot.minute;
      } catch (error) {
  ...
  const handleDeleteAppointment = async () => {
    ...
    if (!window.confirm(`Êtes-vous sûr de vouloir supprimer le rendez-vous #${selectedAppointment.id} ?`)) {
      return;
    }
  ...
  const handleCreateAppointment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json();
        setFormError(error.message || 'Erreur lors de la création du rendez-vous');
      }
    } catch (error) {
      console.error('Erreur:', error);
  ...
  const handleSlotMouseDown = (dayIndex: number, slotIndex: number, e: React.MouseEvent) => {
  ...
  const isSlotInDragSelection = (dayIndex: number, slotIndex: number): boolean => {
  ...
  const loadBlockedSlots = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const newBlockedSlotsData = new Map<string, { id: number; reason: string }>();
        data.forEach((block: { id: number; date: string; time: string; reason: string }) => {
          const key = `${block.date}_${block.time}`;
          newBlockedSlots.add(key);
          newBlockedSlotsData.set(key, { id: block.id, reason: block.reason });
        });
        setBlockedSlots(newBlockedSlots);
  ...
  return (
    <div className="admin-planning-week">
        ...
        <header className="planning-header">
          <div className="header-content">
            ...
            <div className="header-actions">
              <button type="button" onClick={() => setShowCreateModal(true)} className="btn-create">
                + Nouveau RDV
              </button>
            </div>
          </div>
      ...
      <div className="planning-controls">
        ...
        <button onClick={handleCurrentWeek} className="btn-current" disabled={weekOffset === 0}>
          Aujourd'hui
        </button>
                ...
                {weekDays.map((day, dayIndex) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseDown={(e) => !isBlocked && slotAppointments.length === 0 && handleSlotMouseDown(dayIndex, slotIndex, e)}
                      onMouseEnter={() => handleSlotMouseEnter(dayIndex, slotIndex)}
                      onMouseUp={() => !isBlocked && slotAppointments.length === 0 && handleSlotMouseUp(day, slot)}
                      onClick={() => isBlocked && handleBlockedSlotClick(day, slot)}
                      style={{ cursor: isBlocked ? 'pointer' : (slotAppointments.length === 0 ? 'pointer' : 'default') }}
                    >
                      {isBlocked && (
                ...
                <div className="modal-body">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setEditFormData({ ...editFormData, vehicleRegistration: e.target.value })}
                      required
                    />
                  </div>
              ...
              <>
                <div className="modal-body">
                  <div className="detail-row">
                    <strong>Client:</strong> {selectedAppointment.firstName} {selectedAppointment.lastName}
                  </div>
                  <div className="detail-row">
                  ...
                  <div className="detail-row">
                    <strong>Véhicule:</strong> {selectedAppointment.vehicleType} - {selectedAppointment.vehicleBrand} {selectedAppointment.vehicleModel}
                  </div>
                  <div className="detail-row">
                    <strong>Immatriculation:</strong> {selectedAppointment.vehicleRegistration}
                  </div>
                  <div className="detail-row">
      ...
      {showBlockModal && dragStart && dragEnd && (
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p>
                Vous êtes sur le point de bloquer {Math.abs(dragEnd.slot - dragStart.slot) + 1} créneau(x) 
                le {formatDate(weekDays[dragStart.day])}.
              </p>
              <p>Ces créneaux ne seront plus disponibles à la réservation sur le site public.</p>
              <div className="form-group">
      ...
      {showUnblockModal && selectedBlockedSlot && (
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p>
                <strong>Date :</strong> {formatDate(selectedBlockedSlot.date)}<br />
                <strong>Heure :</strong> {selectedBlockedSlot.time}
              </p>
              {selectedBlockedSlot.reason && (
                <p><strong>Raison :</strong> {selectedBlockedSlot.reason}</p>
              )}
              <p className="warning-text">⚠️ Ce créneau redeviendra disponible à la réservation sur le site public.</p>
            </div>
            <div className="modal-footer">
              <button
                type="button"
